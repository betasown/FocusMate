...
export default {
  ...
  if (!picked) return interaction.reply({ content: 'Sélection invalide.' });
      ...
      try {
        ...
        if (!hw) {
          await interaction.followUp({ content: '❌ Devoir introuvable.' });
          return;
        }
        ...
        if (deleted) {
          ...
          await interaction.reply({ content: `✅ Devoir "${hw.title}" (ID: ${picked}) supprimé.` });
        } else {
          await interaction.reply({ content: '❌ Impossible de supprimer le devoir.' });
        }
